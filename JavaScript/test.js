document.addEventListener("DOMContentLoaded", () => {
  ...
  function getFigureBoxValues() {
    ...
    const top = figureRect.top/1.8 ;
    const bottom = figureRect.bottom - 2; // Adjust for the shadow
    const left = figureRect.left -19; // Adjust for the shadow
  ...
  function checkSideCollisions() {
    ...
    platforms.forEach((platform) => {
      ...
      if (
        ...
        const isOnPlatform = playerRect.bottom <= platformRect.top + 4; // we add for a little tolerance to avoid the player getting glitched
